...
The real-run readiness boundary belongs in prompt_runner_sdk, not Atlas. Prompt Runner exposes `packet preflight` JSON, `packet doctor` JSON, and packet planning, and it has validation code for target repo path/git readiness. The bridge behavior is:

1. SDK packet preflight is the authoritative gate for packet-local repo readiness and provider readiness.
2. Setup execution stays explicit in SDK packet config or Atlas workflow config; Atlas must not auto-run discovered `setup.sh` files.
3. Atlas exposes `--preflight-only` and `--skip-preflight` bridge options.
