...
Atlas serializes code-intelligence access per shadow workspace. Lower-level commands use `ATLAS_ONCE_INTELLIGENCE_LOCK_TIMEOUT_SECONDS`. Agent commands use shorter budgets, `ATLAS_ONCE_AGENT_LOCK_TIMEOUT_SECONDS` and `ATLAS_ONCE_AGENT_QUERY_TIMEOUT_SECONDS`, so a stuck backend cannot make other agent calls wait silently. Set `ATLAS_ONCE_INTELLIGENCE_CACHE=0` to disable read-only query caching.
