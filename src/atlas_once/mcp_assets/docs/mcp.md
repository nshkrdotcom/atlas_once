...
On a normal machine, first install Atlas as a tool so `atlas-mcp` is visible from any repo:
...
For local checkout development, update the installed tool from the checkout intentionally:

```bash
uv tool install --reinstall /path/to/atlas_once
```

`uv run atlas ...` is only a checkout development runner. It does not install `atlas-mcp` for
Codex, and Codex will not see the checkout `.venv/bin/atlas-mcp` when launched from another repo.

Register Atlas with Codex:
