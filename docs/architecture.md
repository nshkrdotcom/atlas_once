...
Every command defaults to `--project .` for repo-local use, accepts `--project <ref-or-path>` for cross-repo use, and returns the normal Atlas JSON envelope when `--json` is present. `atlas agent task "<goal>"` composes deterministic freshness, selected symbol searches, ranked files, optional active-file impact context, and next commands without exposing backend flags. It deliberately leaves the slower full map behind explicit `atlas agent map`. Tool metadata records the actual Dexter/Dexterity invocation, retry attempts, cache status, index skip status, and the shadow root used.
