...
When an MCP client is available, prefer `atlas-mcp` tools over shelling out to Atlas commands. Use
read tools first: `atlas_status`, `atlas_config_profile_list`, `atlas_context_ranked_groups`,
`atlas_context_ranked_repos`, `atlas_context_ranked_tree`, `atlas_git_status`, and
`atlas_registry_scan`.

Use write tools only when the user asks for a write and pass `confirm_write=true`. Do not edit
Atlas config files directly; use `atlas_install`, `atlas_config_profile_use`,
`atlas_config_ranked_install`, `atlas_config_ranked_group_add`, or `atlas_mcp_config_install`.

Install or inspect client config with:
...
atlas --json context ranked plan <group> --amount full
atlas --json context ranked status <group>
atlas --json context ranked <group>
atlas --json context ranked tree <group>
atlas --json context ranked <path> --amount mctx-all
```

Use `atlas --json context ranked groups` to discover configured group names and summaries without preparing context. Use `atlas --json context ranked repos <group>` to inspect the resolved repo labels, roots, variants, strategies, and project override counts for a group. For the packaged `nshkrdotcom` profile, default automation should treat `gn-twelve` as the primary workspace group unless the user asks for a different slice. `gn-twelve` is packaged config, not hard-coded command behavior; it reuses the original `gn-ten` variants, `chassis:gn-eleven`, and the budgeted `synapse:gn-twelve` variant. Use `atlas --json context ranked plan gn-twelve --amount full` to inspect selected file counts, estimated tokens, effective options, and cache paths before rendering. Use `atlas --json context ranked tree gn-twelve` when an agent needs repo/project shape for the same ranked set before choosing files; tree JSON includes per-repo and per-project nodes, defaults to source/test/config prefixes, and skips generated or dependency directories. Use the path form for ad-hoc workspace roots that already contain multiple Mix projects.

Ranked context controls are explicit in JSON under `data.effective_options`. `--amount tiny|small|medium|large|full|mctx-all` is the simple sizing layer. Use `--portion 0..100`, `--projects preset|all|included|current`, `--files lib|all-source|all`, `--select ranked|deterministic|full`, `--max-tokens`, `--max-bytes`, and `--no-budget` when a task needs exact behavior. `--amount mctx-all` is the full Mix-project context mode.
...
Render, status, tree, plan, and cache auto-prepare when the prepared manifest is missing, stale, or points at deleted files. Use `atlas --json context ranked prepare <group>` only when explicitly prewarming the prepared manifest. Ranked preparation does not run `dexterity.index`; it queries the watcher-maintained index with a bounded timeout only for `--select ranked` and falls back to deterministic local selection if the query is unavailable. Ranked JSON includes `auto_prepared`, `auto_prepare_reason`, `index_freshness`, and `effective_options`; agents should inspect freshness before deciding whether to refresh, wait, or proceed with stale context. The default render path uses `--wait-fresh-ms 0`, so it does not block on indexing unless the caller requests a bounded wait.
