...
atlas config mcp path [--client codex|generic]
atlas config mcp show [--client codex|generic] [--profile <name>]
atlas config mcp install [--client codex|generic] [--profile <name>]
atlas config mcp doctor [--client codex|generic] [--profile <name>]
...
atlas config mcp install --json
atlas config mcp show --json
atlas config mcp path --json
atlas config mcp doctor --json
```

`atlas-mcp` is the stdio MCP server. `atlas mcp tools` lists the schema-backed tool registry,
including read/write classification. `atlas config mcp install` writes an Atlas-owned client
